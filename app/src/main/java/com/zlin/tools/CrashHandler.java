...
/**
 * Created by zhanglin03 on 2018/12/29.
 */
...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private void dumpExceptionToSDCard(Throwable e) throws IOException{
        ...
        File dir = new File(Constant.ERROR_PATH);
        //如果目录下没有文件夹，就创建文件夹
        if (!dir.exists()) {
            dir.mkdirs();
        }
        ...
        //在定义的Crash文件夹下创建文件
        File file = new File(Constant.ERROR_PATH+File.separator + FILE_NAME + time + FILE_NAME_SUFFIX);
    ...
    private void uploadExceptionToServer() {
        File file = new File(Constant.ERROR_PATH);
        if(!file.exists()){
            return;
        }
